...
import { Wallet, Plus, Home, Truck, AlertCircle } from "lucide-react";
...
const RequestSheet = ({ open, onOpenChange, selectedDocument }) => {
  ...
  const deliveryAvailable = useMemo(
    () => selectedDocument?.deliveryAvailable === true,
    [selectedDocument]
  );

  const digitallyAvailable = useMemo(
    () => selectedDocument?.digitallyAvailable === true,
    [selectedDocument]
  );
  ...
  const handleSubmitRequest = useCallback(async () => {
    try {
      ...
      let status = "pending";
      if (requestForm.deliveryMethod === "pickup") {
        status = "pickup";
              ...
              {deliveryAvailable && (
                ...
                >
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Pay cash on delivery at your address
                    </p>
                  </div>
                </Button>
              )}
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                        This document is available digitally and can be obtained
                        online. No physical pickup required.
                      </p>
                    </div>
                  </div>
                </div>
